...
function queryCommentSuccessHandle(data,textStatus)
{
	...
	for (var commentParentId in data.comment)
	{
		 ...
		 $.each(data.comment[commentParentId],function(n,json){
			...
			commentTemplate.find("div.media-body a.comment-User").attr("src","/blog/user/" + json.user.username + "/").html(json.user.username).after(" <a class=\"small\" href=\"/blog/user/" + json.toUser.username +"/\">@" + json.toUser.username + "</a>" + json.comment.context);
...
function commitComment(articleId,username)
{
	...
	{	
			var commentTemplate = $("div.comment-template").clone(true);
			commentTemplate.find("div.media-body a.comment-User").after(" <a class=\"small\" href=\"/blog/user/" + json.toUser +"/\">@" + json.toUser + "</a>"+json.message);
...
function commitshortComment(shortArticleId,selfusername,username)
{
	...
	function success(data,textStatus,json)
	{
		...
		shortArticleCommentTemplate.find("div.media-body a.comment-User").after(" <a class=\"small\" href=\"/blog/user/" + json.toUser +"/\">@" + json.toUser + "</a>"+json.message);
