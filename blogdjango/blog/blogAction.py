...
class BlogAction:
			...
			for shortArticle in mshortArticles:
				...
				for comment in queryShortComments:
					...
					if comment.parent_comment == None:
						...
						newComment["comment"] = ModelToJson(comment)
						newComment["user"] = ModelToJson(comment.comment_user)
						newComment["toUser"] = ModelToJson(comment.comment_to_user)
	...
	def queryActicleComment(self,requestContext):
		...
		try:
			if userName == None: ##获取自己的评论
				querycomments = Comment.objects.select_related("comment_user").select_related("comment_to_user").filter(blogtext__id__exact=requestContext["articleId"]).filter(blogtext__userDetail__user=self.user)
			else:
				mUserDetail,access = self.blog_permission_required(userName)
				if access:
					querycomments = Comment.objects.select_related("comment_user").select_related("comment_to_user").filter(blogtext__id__exact=requestContext["articleId"]).filter(blogtext__userDetail__username__exact=userName)
					context["username"] = userName
	...
	def queryShortArticle(self,username=None,echpage=6,startNum=sys.maxint):
		...
		try:
			...
			queryShortComments = ShortComment.objects.select_related("comment_user").select_related("comment_to_user").filter(shortarticle__in=mshortArticles)
			context["shortArticles"] = []
